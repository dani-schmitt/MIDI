...
param(
    [string[]]$Path = @(),
    [string]$Description = "ipMIDI component",
    [string]$SubjectName = "Daniel Schmitt",
    [string]$CertificateThumbprint = $env:IPMIDI_SIGNING_THUMBPRINT,
...
if ($matchingCertificates.Count -gt 1) {
    ...
    throw "Multiple valid code-signing certificates match '$SubjectName': $candidateList. Set IPMIDI_SIGNING_THUMBPRINT for this signing run."
}
