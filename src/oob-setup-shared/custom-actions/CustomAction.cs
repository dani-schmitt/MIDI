    ...
    public class CustomActions
    {
        ...
        private static bool CanActivateMidiServiceTransport(Session session)
        {
            ...
            try
            {
                ...
                session.Log($"CheckIpMidiPrerequisites: MIDI service COM activation returned 0x{result:X8}.");
        ...
        private static bool HasLegacyIpMidiDevice(Session session)
        {
            ...
            {
                session.Log($"CheckIpMidiPrerequisites: SetupDiGetClassDevs failed with {Marshal.GetLastWin32Error()}.");
                return false;
            }
            ...
            try
            {
                for (uint index = 0; ; ++index)
                {
                    ...
                    if (!SetupDiEnumDeviceInfo(deviceInfoSet, index, ref deviceInfoData))
                    {
                        ...
                        {
                            session.Log($"CheckIpMidiPrerequisites: SetupDiEnumDeviceInfo failed with {error}.");
                        }
                        break;
                    }
                    ...
                    bool hardwareIdMatches = hardwareIds
                        ...
                        .Any(id => string.Equals(id.Trim(), "*ipMIDI", StringComparison.OrdinalIgnoreCase));

                    if (hardwareIdMatches || string.Equals(serviceName.Trim(), "ipMIDI", StringComparison.OrdinalIgnoreCase))
                    {
                        session.Log($"CheckIpMidiPrerequisites: Legacy ipMIDI device detected at index {index}.");
                        return true;
                    }
                }
            }
        ...
        private static bool HasLegacyIpMidiInstallation(Session session)
        {
            const string uninstallRoot = @"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall";
            string[] uninstallKeys = { "ipMIDI", "ipMIDI Trial" };
            ...
            foreach (RegistryView view in new[] { RegistryView.Registry64, RegistryView.Registry32 })
            {
                foreach (string uninstallKey in uninstallKeys)
                {
                    ...
                    {
                        session.Log($"CheckIpMidiPrerequisites: Legacy uninstall registration found in {view}: {uninstallKey}.");
                        return true;
                    }
                }
            }

            if (RegistryKeyExists(RegistryHive.LocalMachine, RegistryView.Registry64, @"SYSTEM\CurrentControlSet\Services\ipMIDI"))
            {
                session.Log("CheckIpMidiPrerequisites: Legacy ipMIDI service registration found.");
                return true;
            }

            return HasLegacyIpMidiDevice(session);
        }

        [CustomAction]
        public static ActionResult CheckIpMidiPrerequisites(Session session)
        {
            session.Log("CheckIpMidiPrerequisites: Started");
            ...
            try
            {
                ...
                bool oldDriverFound = HasLegacyIpMidiInstallation(session);
                ...
                session["IPMIDI_OS_SUPPORTED"] = windowsSupported ? "1" : "0";
                session["IPMIDI_SERVICE_ABSENT"] = serviceAbsent ? "1" : "0";
                session["IPMIDI_SERVICE_READY"] = serviceReady ? "1" : "0";
                session["IPMIDI_SERVICE_INCOMPLETE"] = !serviceAbsent && !serviceReady ? "1" : "0";
                session["IPMIDI_OLD_DRIVER_FOUND"] = oldDriverFound ? "1" : "0";

                session.Log(
                    "CheckIpMidiPrerequisites: " +
            ...
            {
                session.Log("ERROR: CheckIpMidiPrerequisites: Exception " + ex);
                session["IPMIDI_OS_SUPPORTED"] = "0";
                session["IPMIDI_SERVICE_ABSENT"] = "0";
                session["IPMIDI_SERVICE_READY"] = "0";
                session["IPMIDI_SERVICE_INCOMPLETE"] = "1";
                session["IPMIDI_OLD_DRIVER_FOUND"] = "0";
                return ActionResult.Success;
            }
        }
        ...
        public static ActionResult ConfigureIpMidiFirewallRules(Session session)
        {
            ...
            try
            {
                string requestedValue = session.CustomActionData.ContainsKey("ConfigureFirewall")
                    ? session.CustomActionData["ConfigureFirewall"]
                    : "1";
                requestedValue = requestedValue.Trim().TrimStart('#', '+');
                bool configureFirewall = requestedValue != "0";
                ...
                using (RegistryKey localMachine = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64))
                using (RegistryKey installer = localMachine.CreateSubKey(
                    @"SOFTWARE\nerds.de\ipMIDI\Installer",
                    RegistryKeyPermissionCheck.ReadWriteSubTree))
                {
                    if (installer == null)
                    {
                        session.Log("ERROR: ConfigureIpMidiFirewallRules: Unable to create Installer registry key.");
                        return ActionResult.Failure;
                    }

                    installer.SetValue("ConfigureFirewall", configureFirewall ? 1 : 0, RegistryValueKind.DWord);
                }

                if (!configureFirewall)
                {
                    session.Log("ConfigureIpMidiFirewallRules: Firewall configuration disabled; existing ipMIDI rules removed.");
                    return ActionResult.Success;
                }
