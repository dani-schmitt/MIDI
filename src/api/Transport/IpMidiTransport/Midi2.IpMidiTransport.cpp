...
HRESULT
CMidi2IpMidiTransport::Activate(
    REFIID Riid,
    void **Interface
)
{
    ...
    if (__uuidof(IMidiBidirectional) == Riid)
    {
        ...
        RETURN_IF_FAILED(Microsoft::WRL::MakeAndInitialize<CMidi2IpMidiBidi>(&midiBidi));
    ...
    else if (__uuidof(IMidiServiceTransportPluginMetadataProvider) == Riid)
    {
        ...
        RETURN_IF_FAILED(Microsoft::WRL::MakeAndInitialize<CMidi2IpMidiPluginMetadataProvider>(&metadataProvider));
